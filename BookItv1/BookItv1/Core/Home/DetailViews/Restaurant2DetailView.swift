...
struct Restaurant2DetailView: View {
    @Environment(\.presentationMode) var presentationMode
    @State private var isChatViewActive = false
    var body: some View {
        VStack(spacing: 16) {
            Image("restaurant2Logo")
                .resizable()
            ...
            VStack(alignment: .center, spacing: 8) {
                Text("Кафе корейской и японской кухни")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                Text("ул. Кабанбай Батыра, 140")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                Text("микрорайон Орбита-2, 6")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
            }

            HStack(spacing: 20) {
                Link("Instagram", destination: URL(string: "https://www.instagram.com/kimchi.almaty/")!)
                    .font(.subheadline)
                    .foregroundColor(.blue)

                Link("WhatsApp", destination: URL(string: "https://wa.me/77758880049")!)
                    .font(.subheadline)
                    .foregroundColor(.blue)
            }
            .padding()
...
struct Restaurant2DetailView_Previews: PreviewProvider {
    static var previews: some View {
        Restaurant2DetailView()
    }
}
