...
struct Restaurant3DetailView: View {
    @Environment(\.presentationMode) var presentationMode
    @State private var isChatViewActive = false
    var body: some View {
        VStack(spacing: 16) {
            Image("restaurant3Logo")
                .resizable()
            ...
            VStack(alignment: .center, spacing: 8) {
                Text("Гастрономическая кухня")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                Text("Ресторан, кафе, бар, паб")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                Text("ул. Сатпаева, 29/6А")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
            }

            HStack(spacing: 20) {
                Link("Instagram", destination: URL(string: "https://www.instagram.com/mode_restaurant/")!)
                    .font(.subheadline)
                    .foregroundColor(.blue)

                Link("WhatsApp", destination: URL(string: "https://wa.me/77017669111")!)
                    .font(.subheadline)
                    .foregroundColor(.blue)
            }
            .padding()
...
struct Restaurant3DetailView_Previews: PreviewProvider {
    static var previews: some View {
        Restaurant3DetailView()
    }
}
